...
void CAN1_RX0_IRQHandler(void)
{
  ...
  Can_Receive=temp_rxbuf[0];
	Remoter_Ch1=0,Remoter_Ch2=0; //接收到信号之后 屏蔽航模遥控
	if(Can_Receive==0x5A)	Flag_Qian=0,Flag_Hou=0,Flag_Left=0,Flag_Right=0;//////////////刹车
	else if(Can_Receive==0x41)	Flag_Qian=1,Flag_Hou=0,Flag_Left=0,Flag_Right=0;//////////////前
	else if(Can_Receive==0x45)	Flag_Qian=0,Flag_Hou=1,Flag_Left=0,Flag_Right=0;//////////////后
	else if(Can_Receive==0x42||Can_Receive==0x43||Can_Receive==0x44)	
												Flag_Qian=0,Flag_Hou=0,Flag_Left=0,Flag_Right=1;  //左
	else if(Can_Receive==0x46||Can_Receive==0x47||Can_Receive==0x48)	    //右
												Flag_Qian=0,Flag_Hou=0,Flag_Left=1,Flag_Right=0;
	else Can_Receive=0,Flag_Hou=0,Flag_Left=0,Flag_Right=0;//////////////刹车
	
		
	if(Can_Receive==0x59)  Flag_sudu=2;  //低速挡（默认值）
	if(Can_Receive==0x58)  Flag_sudu=1;  //高速档 
	}
}
#endif
